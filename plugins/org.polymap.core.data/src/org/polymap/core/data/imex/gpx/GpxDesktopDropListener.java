 ...
 */
public class GpxDesktopDropListener
        implements DesktopDropListener {

    private static Log log = LogFactory.getLog( GpxDesktopDropListener.class );
    ...
    public String onDrop( List<DesktopDropEvent> events ) {
        for (DesktopDropEvent ev : events) {
            if (ev instanceof FileDropEvent) {
                ...
                if (fev.getFileName().endsWith( ".gpx" )) {
                    try {
                        IWorkbench workbench = PlatformUI.getWorkbench();
                        IWizardDescriptor descriptor = workbench.getImportWizardRegistry().findWizard( GpxImportWizard.ID );
                        final GpxImportWizard wizard = (GpxImportWizard)descriptor.createWizard();
